    ...
    return (
        <Suspense fallback={
            <div className="min-h-screen bg-base flex items-center justify-center font-body">
                <div className="animate-pulse text-emerald text-lg">Loading experience...</div>
            </div>
        }>
            <SendGiftContent />
        </Suspense>
...
function SendGiftContent() {
    ...
    const inputClass = "w-full bg-transparent border-b border-gray-300 py-3 text-lg font-body focus:outline-none focus:border-emerald transition-colors placeholder-gray-400 rounded-none";

    return (
        <div className="min-h-screen flex flex-col lg:flex-row bg-base font-body">
            {/* Left Pane - Visual & Context */}
            <div className="relative w-full lg:w-[45%] bg-emerald overflow-hidden flex flex-col p-8 lg:p-16 text-base shrink-0 lg:sticky lg:top-0 lg:h-screen">
                {/* Ambient glow effects */}
                <div className="absolute -top-[20%] -left-[20%] w-[70%] h-[70%] rounded-full bg-emerald-muted blur-[120px] opacity-60 mix-blend-screen pointer-events-none"></div>
                ...
                <div className="relative z-10 flex-1 flex flex-col justify-center mt-8 lg:mt-0">
                    <div className="animate-in fade-in slide-in-from-left-8 duration-700 delay-150 fill-mode-both">
                        <h1 className="text-4xl lg:text-6xl font-display leading-tight mb-6 text-white">
                            The perfect <br />
                            <span className="italic text-emerald-light">gesture.</span>
                        </h1>
                        ...
                        {gift ? (
                            <div className="mt-12 backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl transition-transform hover:scale-[1.02] duration-500">
                                <div className="uppercase tracking-widest text-xs font-bold text-emerald-light mb-4">Selected Gift</div>
                            ...
                            <div className="mt-12 p-8 border border-dashed border-white/30 rounded-3xl text-center backdrop-blur-sm bg-white/5">
                                <p className="text-white/70 mb-4">No gift selected yet.</p>
                                <button onClick={() => router.push('/marketplace')} className="bg-white text-emerald px-6 py-3 rounded-full font-bold hover:bg-emerald-light transition-colors active:scale-95">
                                    Browse Marketplace
                                </button>
                            </div>
                        )}
                    ...
                    {steps.map((s) => (
                        <div key={s.id} className="flex-1 flex flex-col gap-3">
                            <div className={`h-1 rounded-full transition-all duration-500 ${
                                s.id === currentStep ? 'bg-emerald-light' : 
                                s.id < currentStep ? 'bg-white/50' : 'bg-white/10'
                            }`}></div>
                            <span className={`text-xs font-bold uppercase tracking-widest ${
                                s.id === currentStep ? 'text-emerald-light' : 'text-white/40'
                            }`}>{s.title}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="w-full max-w-lg mx-auto relative">
                    ...
                    {currentStep === 1 && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <h2 className="text-3xl font-display text-emerald mb-2">Select a Gift</h2>
                            ...
                            {!gift ? (
                                <div className="text-center py-16 bg-base rounded-3xl border border-gray-100">
                                    ...
                                    <button onClick={() => router.push('/marketplace')} className="bg-emerald text-white px-8 py-3.5 rounded-full font-bold hover:bg-emerald-muted transition-all active:scale-95 shadow-lg shadow-emerald/20">
                                        Explore Marketplace
                                    </button>
                                </div>
                            ) : (
                                <div className="flex justify-end mt-12">
                                    <button onClick={nextStep} className="bg-emerald text-white px-8 py-4 rounded-full font-bold hover:bg-emerald-muted transition-all active:scale-95 shadow-lg shadow-emerald/20 flex items-center gap-2">
                    ...
                    {currentStep === 2 && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <h2 className="text-3xl font-display text-emerald mb-2">Recipient Details</h2>
                            ...
                            <div className="flex justify-between items-center mt-12 pt-8 border-t border-gray-100">
                                <button onClick={prevStep} className="text-gray-400 hover:text-emerald font-bold transition-colors">
                                    &larr; Back
                                </button>
                                <button onClick={nextStep} disabled={!formData.recipientName || !formData.recipientEmail || !formData.recipientCountry} className="bg-emerald text-white px-8 py-4 rounded-full font-bold hover:bg-emerald-muted transition-all active:scale-95 shadow-lg shadow-emerald/20 disabled:opacity-50 disabled:cursor-not-allowed">
                                    Continue
                                </button>
                            </div>
                        </div>
                    ...
                    {currentStep === 3 && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <h2 className="text-3xl font-display text-emerald mb-2">Personal Touch</h2>
                            <p className="text-gray-500 mb-10">Add a meaningful note to accompany your gift.</p>
                            
                            <div>
                                <textarea 
                                    id="message" 
                                    className={`${inputClass} text-2xl !py-6 min-h-[200px] resize-none leading-relaxed bg-base/50 rounded-3xl px-8 border-none focus:ring-1 focus:ring-emerald`}
                            ...
                            <div className="flex justify-between items-center mt-12 pt-8 border-t border-gray-100">
                                ...
                                <button onClick={nextStep} className="bg-emerald text-white px-8 py-4 rounded-full font-bold hover:bg-emerald-muted transition-all active:scale-95 shadow-lg shadow-emerald/20">
                                    Review & Pay
                                </button>
                            </div>
                        </div>
                    ...
                    {currentStep === 4 && gift && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <h2 className="text-3xl font-display text-emerald mb-2">Review & Complete</h2>
                            ...
                            <div className="space-y-8 mb-10">
                                <div className="bg-base p-6 rounded-3xl">
                                    <div className="flex justify-between items-end mb-4 border-b border-gray-200 pb-4">
                                        ...
                                        <span className="font-mono text-4xl text-emerald">${calculateTotal(Number(gift.price)).toFixed(2)}</span>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Recipient</span>
                                            <span className="text-sm font-medium text-emerald">{formData.recipientName}</span>
                                        </div>
                                        <div>
                                            <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Delivery</span>
                                            <span className="text-sm font-medium text-emerald">{formData.deliveryDate || 'Immediate'}</span>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <button
                                                ...
                                                className={`relative overflow-hidden p-6 text-left rounded-3xl transition-all duration-300 border-2 ${
                                                    formData.paymentMethod === method 
                                                        ? 'border-emerald bg-emerald/5 scale-[0.98]' 
                                                        : 'border-gray-100 bg-white hover:border-gray-300 hover:bg-gray-50'
                                                }`}
                                            >
                                                <div className={`text-lg font-bold mb-1 ${formData.paymentMethod === method ? 'text-emerald' : 'text-gray-600'}`}>
                                                ...
                                                {formData.paymentMethod === method && (
                                                    <div className="absolute top-4 right-4 w-6 h-6 rounded-full bg-emerald flex items-center justify-center text-white text-xs shadow-md">✓</div>
                                                )}
                                            </button>
                                        ))}
                                    </div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <input type="text" id="cardNumber" className="w-full bg-transparent border-b border-gray-300 py-3 pl-10 text-lg font-body focus:outline-none focus:border-emerald transition-colors placeholder-gray-300" placeholder="0000 0000 0000 0000" value={formData.cardNumber} onChange={handleInputChange} maxLength={19} />
                                            ...
                                            <div className="grid grid-cols-2 gap-6">
                                                <div>
                                                    ...
                                                    <input type="text" id="cardExpiry" className="w-full bg-transparent border-b border-gray-300 py-3 text-lg font-body focus:outline-none focus:border-emerald transition-colors placeholder-gray-300" placeholder="MM/YY" value={formData.cardExpiry} onChange={handleInputChange} maxLength={5} />
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">CVC</label>
                                                    <input type="text" id="cardCvc" className="w-full bg-transparent border-b border-gray-300 py-3 text-lg font-body focus:outline-none focus:border-emerald transition-colors placeholder-gray-300" placeholder="123" value={formData.cardCvc} onChange={handleInputChange} maxLength={4} />
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Name on Card</label>
                                                <input type="text" id="cardName" className="w-full bg-transparent border-b border-gray-300 py-3 text-lg font-body focus:outline-none focus:border-emerald transition-colors placeholder-gray-300" placeholder="JANE DOE" value={formData.cardName} onChange={handleInputChange} />
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {formData.paymentMethod === 'crypto' && (
                                    <div className="animate-in fade-in slide-in-from-top-4 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm mt-6 text-center">
                                        {!formData.walletConnected ? (
                                            <>
                                                <div className="w-16 h-16 bg-emerald/5 rounded-full flex items-center justify-center mx-auto mb-4">
                                                    <svg className="w-8 h-8 text-emerald" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
                                                </div>
                                            ...
                                            <>
                                                <div className="w-16 h-16 bg-emerald/10 rounded-full flex items-center justify-center mx-auto mb-4">
                                                    <svg className="w-8 h-8 text-emerald" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                                                </div>
                            ...
                            <div className="flex flex-col gap-4 mt-12 pt-8 border-t border-gray-100">
                                <button 
                                    ...
                                    className="w-full bg-emerald text-white px-8 py-5 rounded-full font-bold text-lg hover:bg-emerald-muted transition-all active:scale-95 shadow-xl shadow-emerald/20 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-3"
                                >
                                    {sending ? (
                                        <span className="animate-pulse">Processing...</span>
                                    ) : (
                                        <>Send Gift Securely <span className="text-emerald-light">&rarr;</span></>
                                    )}
                                </button>
                                <button 
                                    onClick={prevStep}
                                    disabled={sending}
                                    className="text-gray-400 hover:text-emerald font-bold transition-colors py-2"
                                >
                                    Back to Message
                                </button>
                            </div>
